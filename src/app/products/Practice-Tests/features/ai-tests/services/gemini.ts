  ...
  try {
    ...
    try {
      ...
      const questions = testData.questions.map((q: any) => ({
        ...
        hints: formData.includeHints ? q.hints || [] : undefined,
        resources: formData.includeResources ? q.resources || [] : undefined
      }));
      ...
      throw new Error('Failed to generate valid test data');
    }
  } catch (error) {
